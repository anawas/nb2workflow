...
def understand_comment_references(
    ...
    # this allows to use simplified syntax in some cases, e.g.
    # when just a url alone is provided to indicate type
    comment = re.sub(r"\b(http.*?)(?:\s|$)", r"<\1>", comment)
    comment = re.sub(r"([0-9])\.( |$)", r"\1.0\2", comment)
